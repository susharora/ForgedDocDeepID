...
Visual audit
------------
For every evaluated image the script writes:

    clean overlay PNG
    adversarial overlay PNG
    paired clean -> adversarial comparison PNG

Both overlays use the exact frozen clean-RMA visual convention:

    WHITE   = top-positive CAM contour
    CYAN    = altered face GT
    MAGENTA = altered text GT
    GREY    = document-content boundary

The paired panel also prints p_attack, margin, E, mu_w and PG for clean and
adversarial states. The white contour is visual-only and is not used by the
...
def visual_output_paths(
    row,
):
    split = clean_rma.safe_name(
        str(
            row[
                "evaluation_split"
            ]
...
def add_metrics_header(
    ...
    *,
    ...
    draw.text(
        ...
        fill=clean_rma.COLOR_WHITE,
        font=font,
    )

    line_1 = (
        f"p_attack={probability:.4f}  "
        f"margin={margin:.3f}"
    )
    ...
    if (
        ...
        label = (
            f"E degradation="
            f"{100.0 * relative_E_degradation:.1f}%"
        )
...
def make_paired_overlay_panel(
    ...
    *,
    ...
    right = add_metrics_header(
        adv_overlay,
        "CLASS-PRESERVING LOCALISATION ATTACK",
        probability=adv_probability,
        margin=adv_margin,
        E=E_adv,
        mu_w=mu_adv,
        PG=PG_adv,
...
def render_and_save_visual_audit(
    *,
    row,
    info,
    clean_tensor,
    ...
    paired = (
        make_paired_overlay_panel(
            clean_overlay,
            adv_overlay,
            clean_probability=clean_probability,
            adv_probability=adv_probability,
    ...
    return {
        ...
        "clean_contour_threshold":
            clean_rendered[
                "contour_threshold"
            ],

        "adv_contour_threshold":
            adv_rendered[
                "contour_threshold"
            ],
...
def main():
    ...
    config = {
        ...
        "visual_convention":
            (
                "WHITE CAM top-positive contour; "
                "CYAN altered face GT; "
                "MAGENTA altered text GT; "
                "GREY document boundary"
            ),

        "visual_pngs_per_image":
            3,
    }
    ...
    try:
        for (
            ...
            for (
                ...
                visual = (
                    render_and_save_visual_audit(
                        row=row,
                        info=batch_infos[
                            j
                        ],
                        clean_tensor=batch_x[
                            j
                        ].detach().cpu(),
                        adv_tensor=adv_x[
                            j
                        ].detach().cpu(),
                        clean_cam=clean_eval[
                            "full_cam"
                        ][
                            j
                        ]
    ...
    if (
        ...
        raise RuntimeError(
            "Visual PNG count mismatch"
            f"\nexpected={expected_visual_pngs}"
            f"\nactual={visual_png_count}"
        )
    ...
    print(
        "visual audit: paired clean -> adversarial "
        "overlays for every image"
    )

    print(
        "visual convention: WHITE CAM contour; "
        "CYAN face GT; MAGENTA text GT; "
        "GREY document boundary"
    )

    print(
        f"visual PNGs written: "
        f"{visual_png_count}"
    )
